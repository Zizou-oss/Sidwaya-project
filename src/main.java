...
public class main extends JFrame {
    ...
    private void createTables() throws SQLException {
        String createStagiairesTable = "CREATE TABLE IF NOT EXISTS stagiaires (" +
                ...
                "date_debut TEXT, " +
                "duree TEXT, " +
                "date_fin TEXT, " +  // 🔹 Ajout de date_fin
                "statut TEXT)";
    ...
    private void insertSampleData() throws SQLException {
        try (Statement stmt = connection.createStatement();
            ...
            if (rs.next() && rs.getInt(1) == 0) {
                String insertStagiaire = "INSERT INTO stagiaires (nom, prenom, theme, departement, fonction, date_debut, duree, date_fin, statut) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                try (PreparedStatement pstmt = connection.prepareStatement(insertStagiaire)) {
                    ...
                    pstmt.setString(8, "2024-04-15"); // 🔹 Date de fin exemple
    ...
    private void createModernTable() {
        ...
        dataTable.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (row >= 0 && col == dataTable.getColumnCount() - 1) {
            ...
            try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                switch(currentModule) {
                    case "Stagiaires":
                        ...
                        pstmt.setString(8, getFieldValue(dialog, 7)); // Date fin  🔹
                        pstmt.setString(9, getComboValue(dialog, 8)); // Statut
                        break;
                    case "Décoration":
                        pstmt.setString(1, getFieldValue(dialog, 0));
                        pstmt.setString(2, getFieldValue(dialog, 1));
                        pstmt.setString(3, getFieldValue(dialog, 2));
                        pstmt.setString(4, getComboValue(dialog, 3));
                        pstmt.setString(5, getFieldValue(dialog, 4));
                        pstmt.setString(6, getFieldValue(dialog, 5));
                        pstmt.setString(7, getComboValue(dialog, 6));
                        pstmt.setString(8, getComboValue(dialog, 7));
                        break;
                    case "Formation":
                        pstmt.setString(1, getFieldValue(dialog, 0));
                        pstmt.setString(2, getFieldValue(dialog, 1));
                        pstmt.setString(3, getFieldValue(dialog, 2));
                        pstmt.setString(4, getTextAreaValue1(dialog, 3));
                        pstmt.setString(5, getComboValue(dialog, 4));
                        break;
                }
                pstmt.executeUpdate();
                return true;
    ...
    private String getTextAreaValue1(JDialog dialog, int index) {
    ...
    private String getInsertSQL(String module) {
        switch(module) {
            case "Stagiaires":
                return "INSERT INTO stagiaires (nom, prenom, theme, departement, fonction, date_debut, duree, date_fin, statut) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            case "Décoration":
    ...
    private void addModernStagiaireFields(JPanel panel, GridBagConstraints gbc) {
        String[] labels = {"Nom:", "Prénom:", "Thème:", "Département:", "Fonction:", "Date début:", "Durée:", "Date fin:", "Statut:"}; // 🔹 Ajout de "Date fin:"
    ...
    private void updateTableColumns(String module) {
        ...
        switch(module) {
            case "Stagiaires":
                columns = new String[]{"Nom", "Prénom", "Thème", "Département", "Fonction", "Date début", "Durée", "Date fin", "Statut", "Actions"}; // 🔹 Ajout "Date fin"
                break;
            case "Décoration":
                ...
                while (rs.next()) {
                    ...
                    switch(module) {
                        case "Stagiaires":
                            ...
                            row[7] = rs.getString("date_fin"); // 🔹 Récupération
    ...
    private void performSearch() {
        ...
        try {
            ...
            switch(currentModule) {
                case "Stagiaires":
                    sql += "LOWER(nom) LIKE ? OR LOWER(prenom) LIKE ? OR LOWER(theme) LIKE ? OR LOWER(departement) LIKE ? OR LOWER(fonction) LIKE ? OR LOWER(date_debut) LIKE ? OR LOWER(duree) LIKE ? OR LOWER(date_fin) LIKE ?";
                    break;
                case "Décoration":
    ...
    private void handleModifyAction(int row, String itemName) {
        try {
            Object[] currentData = new Object[dataTable.getColumnCount() - 1];
    ...
    private JTextField[] createFormFields(JPanel formPanel, GridBagConstraints gbc, Object[] currentData) {
        ...
        switch(currentModule) {
            case "Stagiaires":
                ...
                fieldsList.add(addFormField(formPanel, gbc, "Date fin:", row++, (String)currentData[7])); // 🔹 Ajout
                fieldsList.add(addComboField(formPanel, gbc, "Statut:", row++,
                        new String[]{"En cours", "À venir", "Planifiés", "Terminés"},
                        (String)currentData[8]));
                break;
            case "Décoration":
                fieldsList.add(addFormField(formPanel, gbc, "Nom:", row++, (String)currentData[0]));
                fieldsList.add(addFormField(formPanel, gbc, "Prénom:", row++, (String)currentData[1]));
    ...
    private String getUpdateSQL(String module) {
        switch(module) {
            case "Stagiaires":
                return "UPDATE stagiaires SET nom=?, prenom=?, theme=?, departement=?, fonction=?, date_debut=?, duree=?, date_fin=?, statut=? WHERE nom=? AND prenom=?";
            case "Décoration":
            ...
            try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                switch(currentModule) {
                    case "Stagiaires":
                        ...
                        pstmt.setString(8, fields[7].getText().trim()); // 🔹 Date fin
                        pstmt.setString(9, fields[8].getText().trim());
                        pstmt.setString(10, (String) originalData[0]);
                        pstmt.setString(11, (String) originalData[1]);
                        break;
                    case "Décoration":
                        pstmt.setString(1, fields[0].getText().trim());
                        pstmt.setString(2, fields[1].getText().trim());
